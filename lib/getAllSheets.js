...
async function authorize() {
  ...
  if (process.env.GOOGLE_SERVICE_ACCOUNT) {
    try {
      console.log('✓ getAllSheets: Using Service Account authentication');
      let serviceAccount;
  ...
  // Fallback to OAuth method (legacy)
  let credentials;
  let token;
...
async function getAllCompetitionSheets() {
  try {
    ...
    // Fetch data from all sheets (including 'Deelnemers', excluding 'Sheet1')
    const competitionSheets = allSheets.filter(name => name !== 'Sheet1');

    const results = await Promise.all(
      competitionSheets.map(async (sheetName) => {
        try {
          // For Deelnemers: get all columns (A to J)
          // For games: get columns A (names), B (Fantasy Total Score), C (Beste Mike Score), and D (uitslag)
          const range = sheetName === 'Deelnemers'
            ? `'${sheetName}'!A1:J100`
            : `'${sheetName}'!A19:D32`;
